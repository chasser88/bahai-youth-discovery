...
Return ONLY a JSON object (no markdown, no backticks) with this exact structure:
{
  ...
  async function downloadPDF() {
    ...
    const RIGHT_START = margin + LEFT_COL + PAD; // where right column text starts
    const RIGHT_WIDTH = usable - LEFT_COL - PAD - PAD; // available width for right column

    result["90_day_plan"].forEach((step, i) => {
      // Pre-calculate how many lines action and resource need
      doc.setFontSize(9);
      const actionLines = doc.splitTextToSize(step.action, RIGHT_WIDTH);
      doc.setFontSize(8);
      const resourceLines = doc.splitTextToSize(`→ ${step.resource}`, RIGHT_WIDTH);
      const contentLines = Math.min(actionLines.length, 3) + Math.min(resourceLines.length, 2);
      const boxH = Math.max(26, contentLines * 5.5 + 14);

      checkPage(boxH + 4);
      // Clean white box with green left accent
      doc.setFillColor(255, 255, 255);
      ...
      doc.setFont("helvetica", "bold");
      doc.setTextColor(...ORANGE);
      doc.text(step.week, margin + PAD, y + 8);
      doc.setTextColor(...MID);
      doc.setFontSize(7);
      doc.text(step.focus.toUpperCase(), margin + PAD, y + 14);

      // Vertical divider line
      doc.setDrawColor(...GREEN);
      doc.setLineWidth(0.3);
      doc.line(margin + LEFT_COL, y + 4, margin + LEFT_COL, y + boxH - 4);

      // Right column — action text (wrapped, up to 3 lines)
      doc.setFont("helvetica", "normal");
      ...
      // Resource line (wrapped, up to 2 lines, in green)
      doc.setFontSize(8);
      doc.setTextColor(...GREEN);
      ry += 1;
