...
async function saveToAirtable(name, email, ageBand, answers, result) {
  ...
  try {
    ...
    return { saved: true, recordId: data.recordId || "" };
  } catch (err) {
...
export default function Discovery() {
  ...
  const emailLooksValid = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(userEmail.trim());
  const eligibleAge = ageBand === "18-25" || ageBand === "26-35";
  const canStart = userName.trim().length > 0 && emailLooksValid && eligibleAge;
...
Return ONLY a JSON object (no markdown, no backticks) with this exact structure:
{
  ...
  "affirmation": "powerful personalized closing affirmation"
    ...
    try {
      ...
      setResult(parsed);
      const saveResult = await saveToAirtable(userName, userEmail, ageBand, answers, parsed);
      setSaved(saveResult.saved);
      setRecordId(saveResult.recordId);
      setPhase("result");
    } catch (err) {
            ...
            <div style={{ textAlign: "center", position:"relative", zIndex:1 }}>
              ...
              <div style={{ maxWidth: 340, margin: "0 auto" }}>
                ...
                <input
                  ...
                  style={{ background: "#FFFFFF", border: "2px solid #C9943A", borderRadius: 8, padding: "15px 20px", fontSize: 16, color: "#1A1A1A", width: "100%", outline: "none", textAlign: "center", marginBottom: 18, boxSizing: "border-box", boxShadow: "0 2px 8px rgba(201,148,58,0.15)" }}
                />

                <label style={{display:"block",fontSize:11,color:"#9A9490",marginBottom:8,letterSpacing:3,textTransform:"uppercase",fontWeight:700}}>Email</label>
                <input
                  type="email" placeholder="you@example.com"
                  value={userEmail} onChange={e => setUserEmail(e.target.value)}
                  style={{ background: "#FFFFFF", border: "2px solid #C9943A", borderRadius: 8, padding: "15px 20px", fontSize: 16, color: "#1A1A1A", width: "100%", outline: "none", textAlign: "center", marginBottom: 6, boxSizing: "border-box", boxShadow: "0 2px 8px rgba(201,148,58,0.15)" }}
                />
                <div style={{ fontSize: 11.5, color: "#9A9490", marginBottom: 18, fontStyle: "italic" }}>
                  Used to save your roadmap and connect you with a mentor.
                </div>

                <label style={{display:"block",fontSize:11,color:"#9A9490",marginBottom:10,letterSpacing:3,textTransform:"uppercase",fontWeight:700}}>Your Age</label>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center", marginBottom: 24 }}>
                  {["Under 18", "18-25", "26-35", "Over 35"].map(band => {
                    const sel = ageBand === band;
                    return (
                      <button key={band} onClick={() => setAgeBand(band)}
                        style={{ background: sel ? "#EEF6F3" : "#FFFFFF", border: `2px solid ${sel ? "#1B5E4B" : "#E2DDD8"}`, borderRadius: 50, padding: "9px 18px", color: sel ? "#1B5E4B" : "#5A5450", fontSize: 13, cursor: "pointer", fontWeight: sel ? 700 : 400, transition: "all 0.2s" }}>
                        {sel ? "✓ " : ""}{band}
                      </button>
                    );
                  })}
                ...
                {ageBand === "Under 18" && (
                  <div style={{ background: "#FFFBF4", border: "1px solid #C9943A", borderRadius: 8, padding: "14px 18px", marginBottom: 20, fontSize: 13.5, color: "#8B5E1A", lineHeight: 1.7, textAlign: "left" }}>
                    This programme is designed for youth aged 18 to 35. Please come back when you turn 18 —
                    and in the meantime, speak to your local community about junior youth programmes.
                  </div>
                )}
                {ageBand === "Over 35" && (
                  <div style={{ background: "#FFFBF4", border: "1px solid #C9943A", borderRadius: 8, padding: "14px 18px", marginBottom: 20, fontSize: 13.5, color: "#8B5E1A", lineHeight: 1.7, textAlign: "left" }}>
                    This programme is designed for youth aged 18 to 35. If you'd like to support young people
                    instead, you can <a href="/mentor" style={{ color: "#1B5E4B", fontWeight: 700 }}>apply as a mentor</a>.
                  </div>
                )}
              </div>

              <button onClick={() => canStart && setPhase("questionnaire")} disabled={!canStart}
                style={{ background: canStart ? "#1B5E4B" : "#C8D8D2", color: "#FFFFFF", border: "none", borderRadius: 8, padding: "17px 44px", fontSize: 15, fontWeight: 700, cursor: canStart ? "pointer" : "not-allowed", letterSpacing: 2, textTransform: "uppercase", transition: "all 0.3s", boxShadow: canStart ? "0 4px 14px rgba(27,94,75,0.3)" : "none" }}>
                Begin My Discovery →
              </button>
        ...
        {phase === "result" && result && !result.error && (
          <div style={{ padding: "60px 0" }}>
            ...
            <div style={{ textAlign: "center", padding: "44px 0", borderTop: "2px solid #E2DDD8" }}>
              ...
              {wantsMentor && !acctDone && (
                <div style={{ maxWidth: 460, margin: "0 auto 36px", background: "#FFFFFF", border: "2px solid #1B5E4B", borderRadius: 14, padding: "26px 28px", textAlign: "left" }}>
                  <div style={{ fontSize: 10, letterSpacing: 4, color: "#1B5E4B", marginBottom: 10, textTransform: "uppercase", fontWeight: 700 }}>Get matched with a mentor</div>
                  <p style={{ fontSize: 14.5, color: "#5A5450", lineHeight: 1.75, margin: "0 0 18px" }}>
                    Create a password to save this roadmap and be matched with a Bahá'í mentor
                    whose experience fits your path. You'll sign in with <strong>{userEmail}</strong>.
                  </p>
                  {acctError && (
                    <div style={{ background: "#FDF1EF", border: "1px solid #E8B0A5", color: "#C0503D", borderRadius: 8, padding: "11px 14px", fontSize: 13, marginBottom: 14, lineHeight: 1.6 }}>
                      {acctError}
                    </div>
                  )}
                  <input
                    type="password" placeholder="Choose a password" value={acctPassword}
                    autoComplete="new-password"
                    onChange={e => setAcctPassword(e.target.value)}
                    onKeyDown={e => e.key === "Enter" && createAccount()}
                    style={{ width: "100%", background: "#FFFFFF", border: "2px solid #E2DDD8", borderRadius: 8, padding: "13px 15px", fontSize: 15, color: "#1A1A1A", outline: "none", boxSizing: "border-box", fontFamily: "'Georgia', serif", marginBottom: 6 }}
                  />
                  <div style={{ fontSize: 11.5, color: "#9A9490", marginBottom: 16, fontStyle: "italic" }}>
                    At least 8 characters, including a letter and a number.
                  </div>
                  <button onClick={createAccount} disabled={acctBusy || acctPassword.length < 8}
                    style={{ width: "100%", background: acctBusy || acctPassword.length < 8 ? "#C8D8D2" : "#1B5E4B", color: "#FFFFFF", border: "none", borderRadius: 8, padding: "14px 20px", fontSize: 14.5, fontWeight: 700, cursor: acctBusy || acctPassword.length < 8 ? "not-allowed" : "pointer", fontFamily: "'Georgia', serif" }}>
                    {acctBusy ? "Creating your account…" : "Create my account"}
                  </button>
                </div>
              )}

              {acctDone && (
                <div style={{ maxWidth: 460, margin: "0 auto 36px", background: "#EEF6F3", border: "1px solid #A8D5C2", borderRadius: 12, padding: "22px 26px", textAlign: "left" }}>
                  <div style={{ fontSize: 15, fontWeight: 700, color: "#1B5E4B", marginBottom: 8 }}>✓ Account created</div>
                  <p style={{ fontSize: 14, color: "#5A5450", lineHeight: 1.75, margin: "0 0 16px" }}>
                    Your roadmap is saved. A programme administrator will match you with a mentor,
                    and they'll appear on your dashboard once confirmed.
                  </p>
                  <a href="/dashboard" style={{ display: "inline-block", background: "#1B5E4B", color: "#FFFFFF", borderRadius: 8, padding: "12px 24px", fontSize: 14, fontWeight: 700, textDecoration: "none" }}>
                    Go to my dashboard →
                  </a>
                </div>
              )}
