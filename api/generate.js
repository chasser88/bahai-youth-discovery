...
export default async function handler(req) {
  ...
  try {
    ...
    if (!data.content || !data.content.length) {
      return new Response(JSON.stringify({ error: "Empty response from Anthropic" }), { status: 500, headers: corsHeaders });
    }
    ...
    if (firstBrace === -1 || lastBrace === -1) {
      return new Response(JSON.stringify({ error: "No JSON found in AI response" }), { status: 500, headers: corsHeaders });
    }
